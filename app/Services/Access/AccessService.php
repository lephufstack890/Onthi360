...
class AccessService
{
     ...
     *
     * Route hiện tại chỉ có GET, không có submit thật (form trong Blade
     * chưa có name/method) — không tự thêm route/handler POST mới (ngoài
     * phạm vi). Ở đây chỉ chuẩn bị sẵn: nếu URL có ?code=..., tra mã và trả
     * luôn AccessDecision của OrderActivationService::canActivate() để màn
     * hình có dữ liệu thật hiển thị lý do mã dùng được/không, PHỤC vụ TODO
     * thật của form submit khi được xây (OrderActivationService::activate()).
     */
