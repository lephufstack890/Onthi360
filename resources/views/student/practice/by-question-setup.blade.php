...
@section('content')
    ...
    <a href="{{ route('student.practice.index') }}" class="text-[13px] text-slate-500 mb-4 inline-flex items-center gap-1 hover:text-blue-600">‹ Quay lại Luyện tập</a>
    ...
    <div class="rounded-3xl border border-sky-100 bg-gradient-to-br from-sky-50 via-white to-blue-50 shadow-[0_2px_8px_rgba(0,90,180,.04)] p-5 lg:p-6 mb-4 flex items-center justify-between flex-wrap gap-4">
        <div>
            <p class="text-[13px] text-sky-600 font-semibold"><x-lucide name="target" class="inline h-3.5 w-3.5 shrink-0 align-[-2px]" /> Luyện tập theo câu</p>
            <h2 class="text-xl lg:text-2xl font-bold text-slate-800 mt-1">Luyện từng câu, biết đúng/sai ngay</h2>
            <p class="text-[13px] text-slate-500 mt-1 max-w-xl">
    ...
    @if ($practiceTotal === 0)
        <div class="rounded-3xl border-2 border-dashed border-sky-100 bg-white py-12 text-center">
            <p class="text-4xl mb-2">📭</p>
            <p class="text-[13px] text-slate-500">Kho luyện tập chưa có câu hỏi nào được phát hành.</p>
    ...
    @else
        <form method="POST" action="{{ route('student.practiceByQuestion.start') }}"
              ...
              class="bg-white rounded-3xl border border-sky-100 p-5 lg:p-6 space-y-6">
            @csrf

            {{-- ── Bước 1: dạng câu ── --}}
            <div>
                <div class="flex items-center gap-2 mb-3">
                    <span class="w-6 h-6 rounded-xl bg-blue-100 text-blue-600 text-xs font-bold flex items-center justify-center">1</span>
                    <p class="text-[13px] font-bold text-slate-800">Chọn dạng câu hỏi</p>
                </div>
                ...
                <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2.5">
                    <button type="button" @click="setType('')"
                            ...
                            :class="type === '' ? 'border-blue-400 bg-blue-50 ring-2 ring-blue-200' : 'border-sky-100 hover:border-blue-200'">
                    ...
                    @foreach ($practiceTypes as $t)
                        <button type="button" @click="setType('{{ $t['value'] }}')"
                                @disabled($t['count'] === 0)
                                class="rounded-3xl border p-3 text-left transition disabled:opacity-40 disabled:cursor-not-allowed"
                                :class="type === '{{ $t['value'] }}' ? 'border-blue-400 bg-blue-50 ring-2 ring-blue-200' : 'border-sky-100 hover:border-blue-200'">
            ...
            <div class="border-t border-slate-100 pt-5">
                <div class="flex flex-wrap items-center justify-between gap-2 mb-3">
                    <div class="flex items-center gap-2">
                        <span class="w-6 h-6 rounded-xl bg-sky-100 text-sky-600 text-xs font-bold flex items-center justify-center">2</span>
                        <p class="text-[13px] font-bold text-slate-800">Chọn chuyên đề</p>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <button type="button" x-show="selected.length > 0" x-cloak @click="selected = []"
                                class="text-xs text-slate-400 hover:text-blue-600 transition">Bỏ chọn hết</button>
                    </div>
                </div>

                <template x-if="visibleTags.length === 0">
                    <p class="text-[13px] text-slate-400 py-3">Dạng câu này chưa có chuyên đề nào — cứ bỏ trống, hệ thống sẽ lấy toàn bộ câu của dạng đang chọn.</p>
                </template>

                <div class="flex flex-wrap gap-2">
                    <template x-for="tag in visibleTags" :key="tag.id">
                        <button type="button" @click="toggle(tag.id)"
                                class="inline-flex items-center gap-2 px-3.5 py-2 rounded-full border text-[13px] transition"
                                :class="selected.includes(tag.id) ? 'border-sky-400 bg-sky-50 text-sky-700 font-semibold' : 'border-sky-100 text-slate-600 hover:border-sky-200'">
            ...
            <div class="border-t border-slate-100 pt-5 flex flex-wrap items-center justify-between gap-3">
                ...
                <button type="submit" :disabled="matchCount === 0"
                        class="px-6 py-2.5 rounded-xl bg-blue-600 text-white text-[13px] font-semibold shadow-sm hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition">
                    Bắt đầu luyện ›
                </button>
            </div>
        </form>
