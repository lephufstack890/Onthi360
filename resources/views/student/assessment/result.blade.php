...
@section('content')
    {{-- $attemptModel, $isFinal, $score, $total, $breakdown, $eligibleForReview do
    App\Http\Controllers\Student\AssessmentController truyền vào. --}}
    @php
        ...
        $submittedLabel = isset($attemptModel) && $attemptModel->submitted_at
            ? 'Nộp lúc '.$attemptModel->submitted_at->format('H:i')
    ...
    <div class="rounded-3xl bg-gradient-to-br from-emerald-50 to-sky-50 p-8 mb-6 text-center">
        ...
        <p class="text-sm text-slate-500 mt-2">{{ $submittedLabel }}</p>
        @if (!$isFinal)
    ...
    {{-- CTA đánh giá nhẹ nhàng — không chặn hành trình học (10.1, 9.6); chỉ hiện khi đủ điều kiện. --}}
