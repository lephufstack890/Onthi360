...
@section('content')
    ...
    <x-ws.page-header title="Thời khoá biểu" subtitle="Lịch học gộp từ tất cả lớp bạn đang tham gia, xem theo tuần." />
    ...
    <div class="flex items-center justify-between gap-3 mb-4 flex-wrap">
        ...
        @if ($weekOffset !== 0)
            <a href="{{ route('student.schedule.index') }}" class="text-[13px] text-blue-600 font-medium">Về tuần này</a>
        @endif
    </div>

    @if (! $hasAnyClass)
        <x-ws.empty-state title="Bạn chưa tham gia lớp học nào"
                                    ...
                                    @forelse ($day['sessions'] as $s)
                                        <div class="rounded-xl bg-slate-50 border border-slate-100 p-2.5">
                                            ...
                                            <a href="{{ route('student.classes.show', ['class' => $s['classRoomId'], 'tab' => 'schedule']) }}"
                                               class="block text-xs text-blue-600 font-medium mt-1.5">Xem lớp ›</a>
                                        </div>
                                    @empty
