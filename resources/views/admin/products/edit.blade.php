        ...
        <div class="lg:col-span-2 rounded-3xl border border-sky-100 bg-white shadow-[0_2px_8px_rgba(0,90,180,.04)] p-5 sm:p-6">
            <form method="POST" action="{{ route('admin.products.update', $product->id) }}" enctype="multipart/form-data" class="space-y-4">
                ...
                <div>
                    ...
                    @if ($product->cover_image_path)
                        <div class="mb-2 flex items-center gap-3">
                            <img src="{{ asset('storage/'.$product->cover_image_path) }}" alt="Ảnh bìa hiện tại" class="w-20 h-20 rounded-xl object-cover border border-sky-100">
                ...
                <div class="flex gap-3 pt-2">
                    ...
                    <a href="{{ route('admin.products.show', $product->id) }}" class="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-xl border border-sky-100 bg-white px-4 py-2 text-xs font-bold text-slate-700 transition-colors hover:border-sky-200 hover:bg-sky-50">Huỷ</a>
                </div>
            </form>
        </div>

        <div class="bg-white rounded-3xl border border-blue-200 p-6 space-y-3" x-data="{ open: false, reason: '' }">
            <h3 class="flex items-center gap-2 text-[13px] font-bold text-blue-700"><span><x-lucide name="alert-triangle" class="h-4 w-4" /></span> Xóa tài liệu</h3>
            <p class="text-[13px] text-slate-500">Xóa mềm — quyền truy cập đã cấp trước đó vẫn còn dữ liệu để tra cứu. Bắt buộc nêu lý do (10.4).</p>
            <button type="button" @click="open = !open" class="text-xs font-bold text-blue-600 hover:underline" x-text="open ? 'Đóng' : 'Tôi muốn xóa tài liệu này'"></button>
