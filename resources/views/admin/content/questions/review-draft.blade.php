                ...
                <div class="bg-white rounded-2xl border {{ $d['flagged'] ? 'border-amber-200' : 'border-slate-200' }} p-4"
                    ...
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <div class="flex items-center gap-2">
                            @if ($d['promoted'])
                                <x-status-badge tone="success">✓ Đã vào Kho chung</x-status-badge>
                            @endif
    ...
    @if ($document)
        <div class="rounded-2xl bg-slate-50 border border-slate-100 p-4 mt-6">
            <p class="text-sm text-slate-500 flex items-center gap-2">
                <span>ℹ️</span> Mỗi câu tự chuyển vào Kho chung (dạng Nháp) ngay khi bấm "Lưu câu này" và đủ điều kiện — không cần bước gộp riêng.
            </p>
        </div>
    @endif
@endsection
