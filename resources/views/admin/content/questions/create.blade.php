    ...
    <div x-data="{ type: '{{ old('type', 'mcq') }}' }">
        ...
        <div x-show="['coding', 'mcq', 'fill_blank'].includes(type)" x-cloak class="mb-6">
            ...
            <form method="POST" action="{{ route('admin.content.questions.zipImport') }}" enctype="multipart/form-data"
                ...
                <div class="flex-1 min-w-[240px]">
                    <label class="block text-[13px] font-medium text-indigo-700 mb-1" for="zip_package">
        ...
        <form method="POST" action="{{ route('admin.content.questions.store') }}" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="bg-white rounded-3xl border border-sky-100 p-5 h-fit space-y-4">
                ...
                <button type="submit" class="w-full inline-flex min-h-10 items-center justify-center gap-1.5 rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white shadow-sm shadow-blue-200 transition-colors hover:bg-blue-700">Tạo câu hỏi (Nháp)</button>
            </div>
        </form>
    </div>
