...
@section('content')
    ...
    <x-admin.page-header title="Bảng xếp hạng" icon="bar-chart-3" subtitle="Không trộn số liệu giữa các phạm vi khác nhau; tách biệt hoàn toàn với sao/rating (11.2, 9.1)." />

    <x-admin.table :columns="['Phạm vi', 'Số người xếp hạng', 'Trạng thái', '']">
        @forelse ($boards as $b)
            <tr>
                <td class="px-4 py-3 font-medium text-slate-700">{{ $b['scope'] }}</td>
                <td class="px-4 py-3 text-slate-500">{{ $b['entries'] }}</td>
                <td class="px-4 py-3"><x-admin.badge :tone="$b['tone']">{{ $b['status'] }}</x-admin.badge></td>
