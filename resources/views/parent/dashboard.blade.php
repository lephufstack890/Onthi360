...
@section('content')
    ...
    @forelse ($children as $child)
        <div class="rounded-3xl bg-white border border-sky-100 p-5 mb-6">
            <div class="flex flex-wrap items-center justify-between gap-4 mb-4">
                ...
                <a href="{{ route('parent.children.show', $child['id']) }}" class="text-[13px] text-blue-600 font-medium">Xem chi tiết ›</a>
            </div>
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <x-ws.stat label="Buổi học tới" :value="$child['nextSession']" tone="info" />
                <x-ws.stat label="Điểm danh" :value="$child['attendance']" tone="success" />
                <div class="rounded-3xl border border-sky-100 bg-white shadow-[0_2px_8px_rgba(0,90,180,.04)] p-4 sm:p-5">
                    <x-ws.progress-bar :percent="$child['progress']" label="Tiến độ lớp" tone="brand" />
                </div>
            </div>
        </div>
    @empty
        <x-ws.empty-state title="Chưa liên kết con nào" description="Tạo yêu cầu liên kết để bắt đầu theo dõi lịch học, điểm danh và kết quả của con." actionLabel="Liên kết con" :actionHref="route('parent.children.index')" />
    @endforelse
    ...
    <div class="rounded-3xl border border-sky-100 bg-white shadow-[0_2px_8px_rgba(0,90,180,.04)] p-4 sm:p-5">
        ...
        <ul class="space-y-3">
            ...
            @empty
                <li class="text-[13px] text-slate-400">Chưa có kết quả nào được công bố.</li>
            @endforelse
        </ul>
    </div>
@endsection
