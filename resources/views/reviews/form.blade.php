{{--
  ...
  $type/$targetId do App\Http\Controllers\ReviewController::form() truyền vào (đã qua kiểm
  tra đủ điều kiện — ReviewEligibilityService). Submit thật qua reviews.store, xử lý ở
    ...
    <div class="max-w-2xl mx-auto px-4 py-10">
        ...
        <div class="bg-white rounded-2xl border border-slate-200 p-6">
            ...
            <form method="POST" action="{{ route('reviews.store') }}" class="space-y-5" x-data="{
                    overall: {{ (int) old('overall_rating', 0) }},
                    criteria: { @foreach ($criteria as $key => $label) {{ $key }}: {{ (int) old('criteria_scores.'.$key, 0) }}, @endforeach },
                }">
                @csrf
                ...
                <div>
                    ...
                    <textarea id="comment" name="comment" rows="4" maxlength="1000" class="w-full rounded-lg border border-slate-200 text-sm p-3" placeholder="Nêu trải nghiệm cụ thể của bạn...">{{ old('comment') }}</textarea>
                </div>

                <label class="flex items-start gap-3 text-sm text-slate-600 bg-slate-50 rounded-lg p-3">
                    <input type="checkbox" name="disclosure_ack" value="1" class="mt-0.5" required @checked(old('disclosure_ack'))>
