...
@section('content')
    ...
    <div class="rounded-3xl bg-gradient-to-br from-sky-100 via-white to-rose-50 p-6 lg:p-8 mb-6 flex items-start justify-between gap-4 flex-wrap">
        ...
        {{-- Giáo viên KHÔNG có nút "Sửa cuộc thi" — chỉ Admin mới sửa được thông tin cuộc thi
             (title/lịch/đơn vị tổ chức...), giáo viên (cố vấn) chỉ thêm/sửa kỳ thi bên dưới. --}}
    </div>
                    ...
                    @forelse ($exams as $exam)
                        <details class="py-3 group">
                            ...
                            <div class="mt-3 pl-0 space-y-3">
                                <form method="POST" action="{{ route('teacher.competitions.exams.update', $exam['id']) }}" class="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    @csrf
                                    @method('PUT')
                                ...
                                @if ($exam['entriesCount'] === 0)
                                    <form method="POST" action="{{ route('teacher.competitions.exams.destroy', $exam['id']) }}"
