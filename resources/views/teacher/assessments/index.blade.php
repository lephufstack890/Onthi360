...
@section('content')
    ...
    <div class="rounded-3xl border border-sky-100 bg-gradient-to-br from-sky-50 via-white to-blue-50 shadow-[0_2px_8px_rgba(0,90,180,.04)] p-5 lg:p-6 mb-4 flex items-center justify-between flex-wrap gap-4">
        <div class="flex items-start gap-4">
            ...
            <div>
                ...
                <p class="text-[13px] text-slate-500 mt-1">Đề do bạn tự tạo từ kho câu hỏi riêng — trộn được nhiều kiểu câu (6.3).</p>
            </div>
        </div>
        <a href="{{ route('teacher.assessments.create') }}" class="inline-flex min-h-10 items-center justify-center gap-1.5 rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white shadow-sm shadow-blue-200 transition-colors hover:bg-blue-700 shrink-0">+ Tạo đề mới</a>
    </div>
    ...
    <x-ws.table :columns="['Tên đề', 'Số câu', 'Tổng điểm', 'Đã giao', 'Trạng thái', '']">
