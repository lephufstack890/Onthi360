...
@section('content')
    ...
    <a href="{{ route('teacher.assessments.import') }}" class="text-[13px] text-slate-500 mb-4 inline-flex items-center gap-1 hover:text-blue-600">‹ Quay lại Nhập đề</a>

    <x-ws.page-header title="Rà soát: {{ $documentLabel }}" subtitle="Kết quả trích xuất/OCR là bản nháp — phải rà soát và xác nhận trước khi chuyển vào kho (6.4)." />
                ...
                <div class="bg-white rounded-3xl border {{ $d['flagged'] ? 'border-amber-200' : 'border-sky-100' }} p-4"
                    ...
                    <div class="flex items-center justify-between mb-2">
                        ...
                        <div class="flex items-center gap-2">
                            @if ($d['promoted'])
                                <x-ws.badge tone="success">✓ Đã vào kho câu hỏi</x-ws.badge>
                            @endif
    ...
    @if ($document)
        <div class="rounded-3xl bg-slate-50 border border-slate-100 p-4 mt-6">
            <p class="text-[13px] text-slate-500 flex items-center gap-2">
                <span><x-lucide name="info" class="h-4 w-4" /></span> Mỗi câu tự chuyển vào kho câu hỏi (dạng Nháp) ngay khi bấm "Lưu câu này" và đủ điều kiện — không cần bước gộp riêng.
            </p>
        </div>
    @endif
@endsection
