...
@section('content')
    ...
    <x-ws.page-header title="Tài liệu của tôi" icon="book-open" subtitle="Sách, chuyên đề, bộ đề bạn đã mua hoặc kích hoạt — tải bài tập, học liệu đi kèm và PDF hướng dẫn ngay tại đây." />
                                    ...
                                    <div class="space-y-2">
                                        @foreach ($p['exercises'] as $ex)
                                            <div class="flex items-center justify-between gap-3 px-3 py-2.5 rounded-xl border border-sky-100">
                                                ...
                                                <a href="{{ route('access.resource.exerciseAttachment', [$p['id'], $ex['id'], 'statement']) }}" target="_blank" rel="noopener"
                                                   class="text-xs text-blue-600 font-medium shrink-0">Xem đề bài ↗</a>
                                            </div>
                                        @endforeach
                                    </div>
